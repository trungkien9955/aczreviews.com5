...
class CouponCart extends Model
{
    ...
    public static function get_items(){
        if(Auth::check()){
            $items = CouponCart::with(['product'=>function($query){
    ...
    public static function get_total_price(){
        if(!Auth::check()){
            $sub_total_collection = CouponCart::select('sub_total')->where('session_id', Session::get('session_id'))->pluck('sub_total')->toArray();
            $total_price = array_sum($sub_total_collection);
            return $total_price;
        }else{
            $sub_total_collection = CouponCart::select('sub_total')->where('user_id', Auth::user()->id)->pluck('sub_total')->toArray();
            $total_price = array_sum($sub_total_collection);
            return $total_price;
        }
    }
